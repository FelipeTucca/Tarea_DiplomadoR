---
title: "Análisis exploratorio y estadístico de los datos"
...
En esta gráfica se identificaron datos faltantes para la causa de muerte en peces principalmente por bloom de algas (específicamente años 2011, 2012, 2018 y 2022), en cambio, causa de muerte por baja de OD fue la más frecuente en estos últimos 10 años.

Valores atípicos principalmente para el registro de biomasa muerta por causa de bloom de algas (años 2013, 2015 y 2019). Mientras que para la causa de muerte por oxígeno (OD) sobre la biomasa de *S. salar* se visualizan valores atípicos para los año 2011, 2019 y 2021. Estos valores atípicos demostraron ser casos puntuales en mortalidades de peces siendo posiblemente relacionado con el periodo y ocurrencia de baja de OD o presencia de bloom de algas. Si bien, es un registro estimado de mortalidad por centro (de acuerdo al peso (g) y número de individuos muertos por diferentes causas temporales de OD y bloom de algas), es probable que existan errores en la toma de datos, pero en base a las ordenes de magnitud existe coherencia de eventos puntuales de mortalidad transformarse en outliers dentro de los registros para el barrio. 
...
Para esta gráfica boxplot donde se compara la causa de muerte, agrupando los años desde el 2011 al 2022, se observan datos atípicos para las muertes por bloom de algas y OD. Mayor cantidad de datos atípicos son reportados para la causa por bloom de algas, probablemente siendo una consecuencia de eventos puntuales de mortalidad en el barrio. En general, los valores atípicos estan inclunados a un mayor número de muertos por ambas causas.
...
#### *_Relación entre las causas de mortalidad por bloom y oxígeno disuelto_*

La relación entre las causas de mortalidad por bloom y oxígeno disuelto muestra un patrón de dispersión de la información entre los años 2011 a 2022, sin cumplir un linearidad que relacione directamente la causa de muerte por bloom de algas y el decaimiento del OD.
...
bloom_OD %>% ggplot(aes(x= log_biomasa_OD, y=log_biomasa_bloom))+ geom_point(shape = 19, colour = "steelblue4", size= 3) + labs(x= "Log Mortalidad OD (ton)", y= "Log Mortalidad bloom (ton)")+ theme_gray() + labs( title = "Relación mortalidad OD vs Bloom") + theme_bw()
```
...
No se observa un patrón claro que diferencie la mortalidad causada por bloom de algas y disminución de oxígeno cuando se analiza la variable temporal por semana y mes para el barrio de estudio entre el periodo 2011 al 2022. Casos puntuales son observados para el mes 4 con una alta biomasa muerta de peces siendo causa principal el bloom de algas (año 2021). En tanto, al analizar por semana la biomasa muerta de *Salmo salar* sólo se observa este aumento de mortalidad por causa de bloom de algas para las semana primeras semans (mes 4) del año 2021, demostrando ser un evento puntual para la causa de mortalidad por bloom. 

Al realizar un zoom sobre los registros de mortalidad dentro del año 2021 se oberserva una mayor frecuencia y reporte de mortalidad por causa de OD, sin fluctuar }y presentar registros atípicos de mortalidad por esta causa. Sin embargo, para ese año la mayor biomasa de salmones muertos fue afectada por la acción de bloom de algas, siendo la mayor mortalidad registrada históricamente en este barrio con un log biomasa muerta > 5.  
...
Mortalidad1 %>% mutate(Biomasa_ton= (Peso_g * Muertos)/1000000) %>% mutate(log_biomasa= log(Biomasa_ton)) %>% ggplot(aes(x= Nro, y=log_biomasa, color= Causa))+ geom_point(size=1) + labs(x= "Semanas correlativas", y= "Log biomasa muerta (ton)")+ theme_gray() + labs( title = "Relación biomasa muerta (ton) por semana, periodo 2011-2022") + theme_bw() + scale_y_continuous(limits = c(-8,8))

#Zoom para el años 2021
Mortalidad1 %>% mutate(Biomasa_ton= (Peso_g * Muertos)/1000000) %>% mutate(log_biomasa= log(Biomasa_ton)) %>% ggplot(aes(x= Nro, y=log_biomasa, color= Causa))+ geom_point(size=1) + labs(x= "Semanas correlativas", y= "Log biomasa muerta (ton)")+ theme_gray() + labs( title = "Causa de mortalidad en S. salar para el año 2021") + theme_bw() + scale_y_continuous(limits = c(-8,8)) + scale_x_continuous(limits = c(750, 1069))
```
...
# **Análisis estadísitico de los datos**
