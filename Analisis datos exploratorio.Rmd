...
```{r boxplot, echo=TRUE}
Mortalidad1 %>% ggplot(aes(x= Peso_g, fill=Causa)) + geom_boxplot()
```


```{r relación entre variables, echo=TRUE}
Mortalidad1 %>% ggplot(aes(x= Causa, fill=Causa))+ geom_bar() + labs(x="", y="Muertos")
```
