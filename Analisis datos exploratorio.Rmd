...
Basándose en una prueba de correlación entre las causas de muerte por bloom de algas y disminución de oxígeno disuelto en los último 10 años dentro del barrio en estudio, se obtuvo que no existe correlación entre variables (valor p mayor al 5%), moestrando a su vez este ajuste lineal una baja correlación del 15%. Por lo tanto se acepta la hipótesisi nula (HO).
...
Para resumir la biomasa muerta de *S. salar* a causa del bloom de algas y oxígeno disuelto en su conjunto dentro se generaron tablas que muestran el registro por centro, mes y año. 
...
pander::pander(modelo1_anova1_centro, caption = "ANOVA a una vía de clasificación Centro de cultivo")
```
...
Basado en el ajuste del modelo de ANOVA una vía por criterio de clasificación **mes** nos dice que el modelo general existen diferencias en la biomasa muerta entre meses (p valor mayor 0.05), por lo tanto, se rechaza la **hipótesis nula** que hace referencia a una similitud en biomasa muerta entre meses.
...
pander::pander(modelo2_anova2_mes, caption = "ANOVA a una vía de clasificación mes")
```
...
Basado en el ajuste del modelo de ANOVA una vía por criterio de clasificación **año** nos dice que el modelo general existen diferencias en la biomasa muerta entre años (p valor mayor 0.05), por lo tanto, se rechaza la **hipótesis nula** que hace referencia a una similitud en biomasa muerta entre años.
...
pander::pander(modelo3_anova3_año, caption = "ANOVA a una vía de clasificación año")
```
...
## **Modelo lineal análisis de varianza**
...
lm.aov_biomasa <- lm(log_biomasa ~ Causa + centro_id + mes, data = Mortalidad2)

pander::pander (summary(lm.aov_biomasa), caption = "Modelo de efectos fijos")
```


### Independencia

```{r supuesto de independencia, echo=TRUE}
plot(lm.aov_biomasa$residuals, pch = 1, col= "steelblue1")

dwtest(log_biomasa ~ Causa, data = Mortalidad2, alternative = c("two.sided"), iterations = 15)
```


### Homogeneidad de varianza

```{r homogeneidad de varianza, echo=TRUE}
plot(lm.aov_biomasa, 1, pch = 1, col = "steelblue")

bptest(log_biomasa ~ Causa, data = Mortalidad2, center = "median")
```


### Normalidad

```{r test de normalidad, echo=TRUE}
plot(lm.aov_biomasa, 2, pch = 1, col = "steelblue")

aov_residual_biomasa <- residuals(object = lm.aov_biomasa)
shapiro.test(aov_residual_biomasa)
```

Al realizar las pruebas y representaciones gráficas de los supuestos se determina que existe el cumplimiento de dos de estos (normalidad y homogeneidad de varianza). El supuesto de linealidad no se cumple 
