...
Un ajuste previo de la información presente en la planilla "**mortalidad.xlsx**" se realizó mediante la transformación de datos con la función as.factor(), luego se generaron dos nuevas columnas llamadas "*Causa* (agrupando los factores "Mort_bloom" y "Mort_OD") y "*Muertos*" con la función gather(). Finalmente, se filtraron los valores cero con la función filter(). El ajuste en la base de datos permitió generar una nueva planilla excel llamada "**Mortalidad1.xlsx**" para un trabajo óptimo de la información y posterior realización de las tareas indicadas en la pauta de trabajo.
...
La columna causa de transforma a factor mediante la función as.factor().
...
## **Variación de la variable muertos mediante histogramas**

### *Histograma 1*

Debido a que no se logró visualizar de buena manera la información utilizando histograma (Histograma peces muertos), se procedió a la transformación logarítmica de la variable mortalidad en S. salar para los años comprendidos entre el 2011 al 2022.  
...
Se transforma a valores logaritmo la variable muertos para una mejor visualización de la información (ver Histograma salmones muertos entre 2011 al 2022).
...
Se calculó la biomasa de peces muertos (toneladas) y luego se transformaron a valores logaritmo para una mejor visualización de la información (ver Histograma biomasa muerta (ton) entre los años 2011 al 2022).
...
Debido a que no se logra visualizar de buena manera la información utilizando histograma se procedió a la transformación logarítmica de la mortalidad en S. salar a causa de muertes por bloom de algas y oxígeno disuelto. 
...
Datos muestran que históricamente una mayor cantidad de peces muertos a causa de bloom de algas presentes en la columna de agua.
...
Por otro lado, de acuerdo al registro de mortalidades por centro se observa que 4 centros de cultivos presentan históricamente un número mayor a 100 eventos por mortalidad provocadas por bloom de algas y disminución de oxígeno disuelto.
...
Para el barrio en estudio el mayor número de registros históricos de peces muertos se encuentran durante los años 2015, 2019 y 2021, siendo mayoritariamente reportados durante las estaciones de otoño (mes 3 - mes 4 - mes 5) y primavera (mes 10 - mes 11), es decir, una frecuencia mayor a 100 registros. 
...
Al agrupar los años la mortalidad en S. salar no se observa un patrón que diferencie las causas de mortalidad entre el periodo 2011 al 2022, siendo la mediana, cuartiles y bigotes similares entre ambos boxplot. No obstante, en esta gráfica si se visualizan valores atípicos para ambas causas de mortalidad, siendo mayores para el caso de mortalidad por la acción de bloom de algas.
...
No existe un patrón claro que relacione el registro de causa de muerte en el tiempo sobre la biomasa de S.salar. Otras correlaciones fueron realizadas con la variable año y centro_id, pero tampoco hubo un patrón que relacione la biomasa muerta con el tiempo transcurrido ni el lugar (i.e., centro_id). Para esta correlación se consideró como factor la causa de muerte en S. salar.

```{r Correlación biomasa y nro registro, echo=TRUE}
Mortalidad1 %>% mutate(Biomasa_ton= (Peso_g * Muertos)/1000000) %>% mutate(log_biomasa= log(Biomasa_ton)) %>% ggplot(aes(x= Nro, y=log_biomasa, color=Causa))+ geom_point(size=1) + labs(x= "Nro registro", y= "Log biomasa (ton)")+ theme_gray() + labs( title = "Relación registro de mortalidades y biomasa muerta (ton)") + theme_bw()
```


#### *_Correlación peces muertos y peso entre los años 2011 a 2022_*

Salmones con mayor peso (g) registraron un mayor número mortalidades a causa de bloom de algas y oxígeno disuelto durante el periodo 2011 al 2022 para el barrio en estudio.
...
La estadística descriptiva muestra que entre los años 2011 al 2022 la biomasa muerta promedio para el barrio en estudio es de 7.6 toneladas (mediana equivalente a 0.44 ton), alcanzando un máximo de 1031 toneladas de salmones muertos. El registro total de observaciones fue de 1224.
...
Con respecto al número de salmones muertos para este registro histórico se obtuvo en promedio 3352 peces (mediana 155 muertos), con un rango que va desde 1 a 463.152 salmones muertos. El registro total de observaciones fue de 1224.
...
Finalmente, en relación al peso del salmon del Atlántico (S. salar) muerto en función del tipo de mortalidad (bloom y OD) se calculó en promedio peces de 3 kg con una desviación estándar de 1.5 kg para el periodo comprendido entre los años 2011 al 2022.
...
Debido al alto número de registros históricos (n= 1224) por centro para el barrio en estudio se generó una tabla dinámica mediante la función DT::datatable() lo cual permite realizar búsqueda de información específica dentro de la base de datos *Mortalidad1* (**Tabla 1**).
...
Para resumir la biomasa muerta de S. salar a causa del bloom de algas y oxígeno disuelto para el barrio en estudio (periodo 2011 al 2022) se generó un nuevo objeto llamado *Mortalidad2*. Para este nuevo objeto se realizó estadísitica descriptiva de los datos, calculándose el número de observaciones (centros de cultivos en el barrio), promedio de la biomasa muerta (ton), desviación estándar (DE) y rangos mínimos y máximos.
...
En base la descripción estadística realizado para el objeto *Mortalidad2* se elaboró una nueva tabla con la función kbl(), la cual resume cada parámetro sobre la biomasa muerta (ton) a causa de los factores bloom y oxígeno disuelto por cada centro presente en el barrio (**Tabla 2**).
